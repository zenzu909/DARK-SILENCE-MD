...
async (conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        let privacyMenu = `╭━━〔 *SILENT-SOBX-XMD PRIVACY SETTINGS 🌸* 〕━━┈⊷
┃◈╭─────────────·๏
...
> © POWERD BY SILENTLOVER432 ♥️🌸`;
        ...
        await conn.sendMessage(
            from,
            {
                image: { url: `https://files.catbox.moe/vswco6` }, // Replace with privacy-themed image if available
                caption: privacyMenu,
                contextInfo: {
...
async (conn, mek, m, { from, isOwner, reply }) => {
    ...
    try {
        ...
        reply(`*📋 SILENT-SOBX-MD BLOCKED USERS 👥 FIND ! (${count}):*\n\n${list}`);
    } catch (err) {
        console.error(err);
...
cmd({
    ...
    try {
        ...
        return reply(`SILENT-SOBX-MD BIO FIND! USER ☣️ BIO:\n\n${about.status}`);
    } catch (error) {
